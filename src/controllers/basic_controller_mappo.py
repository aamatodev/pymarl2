...
class BasicMACMAPPO:
    ...
    def forward(self, ep_batch, t, test_mode=False, enable_norm=False):
        ...
        grad_on = th.is_grad_enabled()

        if self.detach_every and ((t % self.detach_every) == 0):
            self.agent.detach_hidden()
            # At detach boundaries it's expected to be a leaf.
            # (No assert here.)
        elif t != 0 and grad_on:
            # When training (grad enabled) and not at a detach boundary,
            # the hidden should still be connected to the graph.
            assert (self.agent.h_in.grad_fn is not None), (
                "RNN hidden was detached between steps. "
                "Ensure your agent.forward()/replace_hidden() do not .detach() "
                "the hidden unless BasicMACMAPPO told them to (detach_every)."
            )
