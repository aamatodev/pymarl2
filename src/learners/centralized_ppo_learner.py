...
class CentralPPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        alive_mask_sum = alive_mask.sum(dim=-1, keepdim=True).clamp_min(1e-9)
        num_alive_agents = th.sum(alive_mask, dim=-1).float()
        avail_actions = avail_actions.byte()

        # --- AFTER: make both full and sliced versions ---
        avail_actions_full = batch["avail_actions"]  # (B, T_full, N, A)
        alive_mask_full = (
                (avail_actions_full[..., 0] != 1.0) *
                (avail_actions_full.sum(dim=-1) != 0.0)
        ).float()  # (B, T_full, N)
        alive_mask_full_sum = alive_mask_full.sum(dim=-1, keepdim=True).clamp_min(1e-9)

        # For actor/rewards/advantages (length T):
        avail_actions = avail_actions_full[:, :-1]  # (B, T, N, A)
        alive_mask = alive_mask_full[:, :-1, :]  # (B, T, N)
        num_alive_agents = alive_mask.sum(dim=-1).float()
        avail_actions = avail_actions.byte()
        ...
        # OLD values
        values_all = self.critic(batch)  # (B, T_full, N)
        # Joint scalar value per timestep (length T_full)
        old_values_before = ((values_all * alive_mask_full).sum(-1) /
                             alive_mask_full_sum.squeeze(-1)).detach()  # (B, T_full)

        if self.is_value_normalized or self.is_popart:
            old_values_before = self.denormalize_value(old_values_before)  # still (B, T_full)
        ...
        # NEW values during critic update
        for _ in range(self.mini_epochs_critic):
            new_values_all = self.critic(batch)[:, :-1, :]  # (B, T, N)
            new_joint_values = (
                    (new_values_all * alive_mask).sum(-1) /
                    alive_mask.sum(dim=-1).clamp_min(1e-9)  # (B, T)
            )
            vf_loss = th.sum((new_joint_values - returns) ** 2 * mask) / mask.sum()
            self.optimiser_critic.zero_grad()
            vf_loss.backward()
            self.optimiser_critic.step()
