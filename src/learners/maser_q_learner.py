...
class maserQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.ind = self.args.ind
        self.mix = self.args.mix
        self.expl = self.args.expl
        self.dis = self.args.dis
        self.goal = "maser"
        self.mixer = None
        ...
        # self.optimiser = th.optim.Adam(self.params, lr=args.lr, eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.goal == "maser":
            ...
            explore_q_target = th.ones(target_ind_q.shape, device=self.device) / target_ind_q.shape[-1]
            ...
            for i in range(batch.batch_size):
                ...
                for j in range(self.n_agents):
                    ...
                    y = F.softmax(target_ind_q[i, ddqn_qval_up_idx[i][j]:, j, :], dim=-1) + th.tensor(1e-6,
                                                                                                      device=self.device)
                    ...
                    cos_sim = cos(target_ind_q[i, :, j, :], goal_q)
                    similarity = th.ones_like(cos_sim, device=self.device) - cos_sim
            ...
            intrinsic_rewards = th.zeros(rewards.shape, device=self.device)
        ...
        if self.goal == "maser":
            ...
            ones_like_terminated = th.ones_like(terminated, device=self.device)
            individual_targets = y * rewards + self.lam * intrinsic_rewards_ind + self.args.gamma * (
                    ones_like_terminated - terminated.repeat(1, 1, target_individual_qvals.shape[
                -1])) * target_individual_qvals
