...
class LAGMALearner:
        ...
        if self.args.use_vqvae:
            for t in range(batch.max_seq_length):
                ...
                if self.args.recon_type == 1:
                    ...
                    vae_loss, ce_loss, vq_loss, commit_loss, coverage_loss = \
                        self.vae.loss_function(state_input, recon, z_e, latent_emb, ndx=ndx, Cqt=Cqt_target,
                                               recon_Cqt=Cqt_hat)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
                ...
                if self.update_vqvae:
                    ...
                    if self.args.flag_batch_wise_vqvae_loss:
                        if self.args.recon_type == 1:
                            ...
                            vae_loss, ce_loss, vq_loss, commit_loss, coverage_loss = \
                                self.vae.loss_function_batch(state_input, recon, z_e, latent_emb, ndx=ndx,
                                                             Cqt=Cqt_target, recon_Cqt=Cqt_hat)
                    ...
                    for t in range(batch.max_seq_length):
                        ...
                        if (t % self.args.trj_update_freq == 0):
                            ...
                            if self.args.buffer_update_ref == 1:  # sum_rewards
                                ...
                                Cq0 = reward_tgo[i, t]
                            seq = visit_nodes[i][t:]
                            self.vae.Buffer_seq[idx0].push(Cq0, seq, flag_des_trj[
                                i])  # insert sequence ( takes time O(log(k)) )
                            idx_prev = idx0
                            ...
                            if (self.args.flag_zero_state_management == False) or (
                                    self.args.flag_zero_state_management == True and idx != self.args.n_codes):
        ...
        targets = self.args.gamma * eta * float(self.args.goal_reward) + rewards + self.args.gamma * (
        ...
        if (
                ...
                and (episode_num - self.last_target_update_step)
